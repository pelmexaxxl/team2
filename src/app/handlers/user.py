...
Ты только что присоединился к нашему боту, и я рад тебя видеть! Я здесь, чтобы помочь тебе с различными задачами. Используй команды, чтобы управлять сбором данных, создавать опросы, отслеживать настроения команды и многое другое!

Теперь у тебя есть доступ ко множеству полезных функций. Вот несколько команд, с которых можно начать:
...
Если что-то будет непонятно или возникнут вопросы, не стесняйся обращаться! Я здесь, чтобы помочь. 🚀
            """
            )
    else:
